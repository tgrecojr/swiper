...
The application must track employee in-office attendance across configurable reporting periods, calculate compliance against configurable minimums (default 20 days), handle exclusion days that reduce requirements, and persist data without database infrastructure.
